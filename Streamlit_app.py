...
def report1(df):
    ...
    summary = (
        ...
        .sort_values(by='Defective Count', ascending=False)
    )

    grand_total = summary['Defective Count'].sum()
...
def report2(df):
    ...
    required_cols = ['OW', 'CAMC', 'IW', 'EW', 'LAMC', 'REPEAT', 'STOCK']
...
def report3(df):
    ...
    pivot["Contribution %"] = (pivot["Grand Total"] / overall_total * 100).round(2)

    pivot = pivot.sort_values("Grand Total", ascending=False)

    grand_total = pivot[required_cols + ["Grand Total"]].sum()
...
def report4(df):
    ...
    required_cols = ["UNIT", "PCB", "SPARE", "CONSUMABLES", "SUB UNIT"]
    ...
    pivot["Contribution %"] = (pivot["Grand Total"] / total * 100).round(2)
...
def report5(df):
    ...
    warranty_cols = ["OW", "CAMC", "IW", "EW", "LAMC", "REPEAT", "STOCK"]

    rows = []

    overall_total = len(df)

    for defect in sorted(df["DEFECTIVE_TYPE"].unique()):
        defect_df = df[df["DEFECTIVE_TYPE"] == defect]
        ...
        for work, row in pivot.iterrows():
            rows.append({
                "DEF_TYPE": defect,
                "TYPE_OF_WORK": work,
                **{c: int(row[c]) for c in warranty_cols},
                "Grand Total": int(row["Grand Total"]),
                "Contribution %": round(row["Grand Total"] / overall_total * 100, 2)
            })
...
def report7(df):
    df = df[df["UNIT_STATUS"].astype(str).str.upper().eq("IW")]
    df = df[df["PRODUCT_MODEL"].notna() & df["DEF_MOD_BRD_NAME"].notna()]
    ...
    for model in sorted(df["PRODUCT_MODEL"].unique()):
        model_df = df[df["PRODUCT_MODEL"] == model]

        counts = model_df["DEF_MOD_BRD_NAME"].value_counts().reset_index()
        counts.columns = ["DEF_MOD_BRD_NAME", "Count"]

        model_total = counts["Count"].sum()

        contribution = round(model_total / overall_total * 100, 2)

        first = True

        for _, r in counts.iterrows():
            rows.append({
                "PRODUCT_MODEL": model if first else "",
                "DEF_MOD_BRD_NAME": r["DEF_MOD_BRD_NAME"],
                "Count": int(r["Count"]),
                "Contribution %": ""
            })
            first = False
        ...
        rows.append({
            "PRODUCT_MODEL": "",
            "DEF_MOD_BRD_NAME": "Grand Total",
            "Count": int(model_total),
...
def report8(df):
    warranty_cols = ["OW", "CAMC", "IW", "EW", "LAMC", "REPEAT"]
...
def report10(df):
    ...
    for model in model_order:
        ...
        pivot = pd.pivot_table(
            model_df,
            index=["DEF_PART_SN", "DEF_MOD_BRD_NAME"],
            columns="UNIT_STATUS",
            aggfunc="size",
            fill_value=0
        )
...
def report11(df):
    ...
    work_cols = sorted(df["TYPE_OF_WORK"].dropna().unique())
    ...
    for model in (
        ...
        for (sn, part), row in pivot.iterrows():
            rows.append({
                ...
                "DEF_PART_SN": sn,
                "DEF_MOD_BRD_NAME": part,
                **{c: int(row[c]) for c in work_cols},
                "Grand Total": int(row["Grand Total"])
            })
            first = False
...
def report12(df):
    ...
    return pivot.sort_values("Grand Total", ascending=False)


def report13(df):
    df = df[
        (df["DEF_TYPE"] == 14) &
        (df["UNIT_STATUS"].astype(str).str.upper().eq("IW"))
    ]
    ...
    return pivot.sort_values("Grand Total", ascending=False).reset_index()
